...
case class TypingMap[
 ...
 def getFailedValues(key: Key): Set[Value] = {
  ...
  override def combineTyping(
    ...
    implicit lazy val monoidMTR: Monoid[Map[Value,TypingResult[Err,Evidence]]] = new Monoid[Map[Value,TypingResult[Err,Evidence]]] {
      def empty: Map[Value,TypingResult[Err,Evidence]] = Map()
      def combine(
        m1: Map[Value,TypingResult[Err,Evidence]],
        m2: Map[Value,TypingResult[Err,Evidence]]): Map[Value,TypingResult[Err,Evidence]] =
        m1 |+| m2
    }
