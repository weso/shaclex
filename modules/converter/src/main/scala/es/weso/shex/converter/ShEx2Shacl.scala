...
/*  private def cnvPrefixMap(pm: PrefixMap): Map[String, IRI] = {
  ...
  private def cnvShapeAnd(shapeAnd: shex.ShapeAnd, id: RDFNode): Result[shacl.Shape] =
    for {
    shapes <- sequence(shapeAnd.shapeExprs.map(shapeExpr2RefNode(_)))
    } yield {
  ...
  for {
    refNodes <- sequence(
      shapeOr.shapeExprs.map(shapeExpr2RefNode(_)))
  } yield {
  ...
  private def cnvFlatShape(
    ...
    Shape.empty(id).addPropertyShapes(p.map(_._2))
 ...
 /* private def cnvConstraints(cs: Vector[Constraint],
  ...
  private def cnvConstraint(
    ...
    if (c.hasExtra || qualified) {
      for {
        ...
        shape <- mkQualifiedPropertyShape(shaclPath, c.shape, c.card.min, c.card.max, sref.id, c.as.getOrElse(List()).map(cnvAnnot))
      } yield (shape,sref)
    } else {
      for {
        sref <- getTripleExprId(c.tc)
        shape <- mkPropertyShape(shaclPath, c.shape, c.card.min, c.card.max, sref.id, c.as.getOrElse(List()).map(cnvAnnot))
      } yield (shape,sref)
    }
  }
  ...
  private def mkPropertyShape(
    ...
    val rs: Result[PropertyShape] = for {
     ...
     propertyShape = Shape.emptyPropertyShape(id, path).copy(
